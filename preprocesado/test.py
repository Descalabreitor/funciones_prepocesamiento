...
class Escalar:
    ...
    def ajustar(self, x, Min=-1, Max=1):
        if type(x[0]) is float or type(x[0]) is int:
            ...
            self.Xmax = np.max(x)
            self.Xmin = np.min(x)
            self.constant = ((Min * self.Xmax) - (Max * self.Xmin)) / (Max - Min)
            self.constant_inv = ((self.Xmin * Max) - (Min * self.Xmax)) / (self.Xmax - self.Xmin)
        else:
            self.tipo_org = []
            self.Xmin = []
            self.Xmax = []
            ...
            for estructura in x:
                ...
                self.constant.append(((Min * np.max(estructura)) - (Max * np.min(estructura))) / (Max - Min))
                self.constant_inv.append(((np.min(estructura) * Max) - (Min * np.max(estructura))) / (
                            np.max(estructura) - np.min(estructura)))
